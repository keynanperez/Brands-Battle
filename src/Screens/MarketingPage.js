...
class App extends React.Component {
  ...
  gettwitterAns=async(questionNumber)=>{
    ...
    const url = `http://10.0.0.25:44300/api/Twitter?Input=`+this.state.chosenbrand+`&question=`+ 1
   ...
   console.log((res.data.public_metrics.followers_count))
...
render(){
  return (
   ...
   <View  style={styles.view} >
       ...
       <Header></Header>
     <SearchBox style={styles.textInput} 
     ...
     renderItem = {({item}) => (
      ...
      <TouchableOpacity onPress={() => this.PressBrand({item})} style={[styles.item]}>
    ...
    {this.state.chosenbrand!=="" ? <View  style={styles.brandInfo} >
     <Image source={{uri: this.state.image_url}}
       style={{width: 80, height: 80}} />
       <Text style={styles.textoutput} >Brand Name:{this.state.chosenbrand}</Text>
       <Text style={styles.textoutput} >Location:{this.state.location}</Text>
       <Text style={styles.textoutput} >NO. of Followers:{this.state.followers}</Text>
       <Text style={styles.textoutput} >Popularty:{this.state.followers}</Text>
       <Text style={styles.textoutput} >NO. of Followers:{this.state.followers}</Text>
     </View>:null}
